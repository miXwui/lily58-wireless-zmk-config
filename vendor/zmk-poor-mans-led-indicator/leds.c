...
extern void led_init_thread(void *d0, void *d1, void *d2) {
    ...
    // ==========================================================================
    // PATCH: Replace fixed 200ms boot delay with polling-based BLE detection
    // Original: k_sleep(K_MSEC(200)); // fixed delay, doesn't handle quick power cycles
    // Patched: Poll every 200ms for up to 5s, breaks early when connected
    // Why: Original fixed delay caused 10 slow blinks on quick power cycle
    //      (turn off then immediately back on) because BLE wasn't connected yet
    //      when the LED module checked. Polling allows the module to detect
    //      connection as soon as it happens.
...
// Patched: boot delay handled by polling in led_init_thread, not fixed delay
